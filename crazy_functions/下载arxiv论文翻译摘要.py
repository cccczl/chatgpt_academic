...
def download_arxiv_(url_pdf):
    ...
    x = x.replace('?', '？')\
        ...
        .replace('  ', ' ')\
        .replace('  ', ' ')
    return f'./gpt_log/arxiv/{title_str}', other_info
...
def get_name(_url_):
    ...
    try:
        ...
        year = re.search(r'(\d{4})', year, re.M | re.I)[1]
...
def 下载arxiv论文并翻译摘要(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    try:
        ...
        report_execption(chatbot, history, a = f"解析项目: {txt}", b="下载pdf文件未成功")
        yield from update_ui(chatbot=chatbot, history=history) # 刷新界面
        return
    ...
    import shutil
    # 重置文件的创建时间
    shutil.copyfile(pdf_path, f'./gpt_log/{os.path.basename(pdf_path)}')
    os.remove(pdf_path)
